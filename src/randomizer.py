        ...
        for file in files:
            if file.endswith(".txt"):
                ...
                if not len(randomlist):
                    continue
                text_file = (
                    os.path.join(root, file).replace(folderpath, "").strip(os.sep)
                )
...
def get_random_word_from_specified_list(
    listname: str, list_dict: dict
) -> Optional[str]:
...
def replace_words_inside_brackets_with_randomizer(
    prompt: str, list_dict: dict, max_depth: int, depth: int = 0
) -> str:
    ...
    for word in identified_words:
        ...
        if replacement:
            prompt = prompt.replace(
                word,
                replace_words_inside_brackets_with_randomizer(
                    replacement, list_dict, max_depth, depth + 1
                ),
                1,
            )
